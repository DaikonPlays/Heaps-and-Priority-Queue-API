 ...
 * Sources used: tutor
 * 
 * Ticket for autograder. Prioritizes
 * certain requests over others.
 */
...
public class Ticket implements Comparable<Ticket>{
	...
	/**
	 * compares the input ticket and the current ticket
	 * @param other input ticket 
	 * @return compareTo between current ticket and input ticket
	 */
	@Override
	public int compareTo(Ticket other){
		Integer otherTicket = orderMap.get(other.ticketType);
		Integer currTicket = orderMap.get(this.ticketType);
		if(currTicket.compareTo(otherTicket) == 0) {
			return this.createdAt.compareTo(other.createdAt);
		}
		else {
			return currTicket.compareTo(otherTicket);
		}
